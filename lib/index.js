...
function _default(babel) {
  ...
  function expression() {
    ...
    do {
      ...
      members = isString(member) ? members.concat(member.split('.').reverse().map(toIdentifier)) : members.concat([member]);
      index--;
    } while (index > -1);
  ...
  function objectAssign(target, members) {
    return t.expressionStatement(t.callExpression( // Object.assign(target, members)
    expression("Object.assign"), [target, t.objectExpression(members)]));
  }

  function objectCreate(parentClass) {
    return t.callExpression( // Object.create(parentClass)
  ...
  function es5Method(method) {
    var id = method.key; // foo: function foo(args) {/* code */}
  ...
  function superCall(path) {
    ...
    targetPath.replaceWith(t.callExpression( // {super target}.apply(this, args)
    caller, [t.Identifier('this')].concat(targetPath.node.arguments)));
  }

  function es5Class(_class) {
    var _es5Class = [],
        MyClass = _class.id; // constructor

    if (!_class.hasOwnProperty('constructor')) {
      _class.constructor = t.functionDeclaration(_class.id, [], t.blockStatement([]));
    }

    _es5Class.push(_class.constructor); // parent class


    if (_class.parentId) {
      // MyClass.prototype = Object.create(MyParentClass.prototype);
      _es5Class.push(assign(expression(MyClass, 'prototype'), objectCreate(expression(_class.parentId, 'prototype'))));
